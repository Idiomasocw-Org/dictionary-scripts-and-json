...
const text = `© Oxford University Press 1 / 4
...
See you soon
sit down
© Oxford University Press 2 / 4The Oxford Phrase List™
someone else
something else
speak to sb
spend time
...
do?
sound like sb/sth
© Oxford University Press 3 / 4The Oxford Phrase List™
start with sb/sth
such a/an...
such a good idea
...
wrong with sb/sth
you see
© Oxford University Press 4 / 4The Oxford Phrase List™
B2
a bit of a...
account for sth
accused of sth
...
const cleanLines = [];
const headersToIgnore = ['A1', 'A2', 'B1', 'B2', 'C1', 'The Oxford Phrase List™'];

for (let line of lines) {
    line = line.trim();
    if (!line) continue;
    if (line.startsWith('© Oxford University Press')) continue;
    if (line.match(/^\d \/ \d/)) continue;
    if (headersToIgnore.includes(line)) continue;
    if (line.startsWith('The Oxford Phrase List is a list of')) continue;
    if (line.startsWith('verbs, compounds, collocations')) continue;

    cleanLines.push(line);
}

fs.writeFileSync('oxford-phrase-level-list.txt', cleanLines.join('\n'), 'utf8');
console.log('✅ Created oxford-phrase-level-list.txt');
